...
def validate_skill(path: Path, runner: Runner = subprocess.run) -> None:
    result = runner([sys.executable, str(path / "scripts" / "validate_skill.py"), str(path)], check=False)
    if result.returncode:
        raise RuntimeError("validação estrutural da skill falhou")


def prepare_staging(skills_dir: Path, *, skip_deps: bool, runner: Runner = subprocess.run) -> Path:
    ...
    try:
        ...
        if not skip_deps:
            ...
            runner([str(python_in(venv_dir)), "-m", "pip", "install", "--quiet", "--disable-pip-version-check", "--upgrade", "pip"], check=True)
            runner([str(python_in(venv_dir)), "-m", "pip", "install", "--quiet", "--disable-pip-version-check", "-r", str(staging / "requirements.txt")], check=True)
            validate_skill(staging, runner)
        return staging
    except Exception:
...
def publish_staging(staging: Path, destination: Path, *, force: bool) -> None:
    ...
    if occupied and not force:
        raise RuntimeError(f"Destino já contém uma skill: {destination}. Revise o conteúdo ou use --force para substituí-la.")
    backup: Path | None = None
    try:
        if occupied:
            backup = destination.parent / f".{SKILL_NAME}.backup-{uuid.uuid4().hex[:10]}"
...
def main() -> None:
    ...
    if args.dry_run:
        print(f"DRY-RUN: validaria e prepararia {SOURCE_SKILL} antes de publicar em {destination}")
        return
    ...
    try:
        ...
        raise SystemExit(f"Instalação abortada sem substituir a instalação anterior: {error}") from error
