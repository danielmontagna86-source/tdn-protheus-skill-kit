...
class SnapshotStore:
    ...
    def read_page(self, page_id: int, manifest: dict[str, Any] | None = None) -> dict[str, Any]:
...
class SnapshotSynchronizer:
    ...
    def snapshot(
        ...
        self._discovered_count = 0
        self._deadline = time.monotonic() + max_duration_seconds if max_duration_seconds is not None else None
                ...
                while pending:
                    ...
                    if record is not None:
                        if len(record["text"]) >= 60:
                            ...
                            state["pages"][str(page_id)] = page_summary(record, status="filtered")
    ...
    def refresh(self, max_depth: int, max_pages: int | None) -> dict[str, Any]:
        with SnapshotLock(self.store.root):
            ...
            stats = {"new": 0, "changed": 0, "unchanged": 0, "removed": 0, "filtered": 0}
            try:
                ...
                for page_id in discovered:
                    ...
                    if previous and not page_changed(previous, version):
                        ...
                        if previous.get("status") == "active":
                            self.store.copy_active_page_to_stage(run_id, page_id, previous_manifest)
                        stats["unchanged"] += 1
                        time.sleep(self.delay)
                        continue
                    record = self.fetch_page(page_id)
