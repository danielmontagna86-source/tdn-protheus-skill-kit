    ...
    def test_prepare_failure_leaves_existing_installation_untouched(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(subprocess.CalledProcessError):
                module.prepare_staging(skills, skip_deps=True, runner=failing_runner)
            self.assertEqual(marker.read_text(), "old")
            self.assertFalse(any(path.name.startswith(f".{module.SKILL_NAME}.staging-") for path in skills.iterdir()))
