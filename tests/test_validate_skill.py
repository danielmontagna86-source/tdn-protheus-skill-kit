...
class ValidateSkillTests(unittest.TestCase):
    ...
    def test_processor_generates_the_jsonl_contract(self) -> None:
        expected_fields = {"id", "source_url", "title", "chunk_index", "total_chunks", "modules", "tables", "parameters", "routines", "entry_points", "target_audience", "content"}
        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            source = temp / "tdn_pages.json"
            output = temp / "chunks.jsonl"
            source.write_text(json.dumps([{"id": 42, "url": "https://tdn.totvs.com/example", "title": "ADVPL e SIGAFAT", "text": "Use MATA410 com a tabela SC5 e o parametro MV_TESTE."}]), encoding="utf-8")
            result = subprocess.run([sys.executable, str(SKILL_DIR / "scripts" / "process_tdn.py"), str(source), str(output)], capture_output=True, text=True, check=False)
            self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
    ...
    def test_scripts_expose_help_without_network(self) -> None:
        for script in ("collect_tdn.py", "locate_tdn_pages.py", "process_tdn.py", "sync_tdn_snapshot.py"):
            result = subprocess.run([sys.executable, str(SKILL_DIR / "scripts" / script), "--help"], capture_output=True, text=True, check=False)
            self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
    ...
    def test_installer_dry_run_reports_target_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run([sys.executable, str(INSTALLER), "--platform", "codex", "--target", str(target), "--dry-run"], capture_output=True, text=True, check=False)
            self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
    ...
    def test_installer_refuses_to_replace_existing_skill_without_force(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run([sys.executable, str(INSTALLER), "--platform", "codex", "--target", str(target), "--skip-deps"], capture_output=True, text=True, check=False)
    ...
    def test_installer_replaces_existing_skill_only_with_force(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run([sys.executable, str(INSTALLER), "--platform", "codex", "--target", str(target), "--skip-deps", "--force"], capture_output=True, text=True, check=False)
            self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
    ...
    def test_package_is_allowlisted_and_contains_no_mcp_or_local_data(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for relative in ("tdn_protheus_mcp/server.py", "tdn-cache/page.json", "saida-local/tdn_pages.json", "sample.jsonl", ".venv/secret"):
                path = package_root / relative
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text("not for distribution", encoding="utf-8")
            output = package_root / "dist" / "release.zip"
            result = subprocess.run([sys.executable, str(package_root / "package.py"), "--output", str(output)], capture_output=True, text=True, check=False)
            self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
            with zipfile.ZipFile(output) as archive:
                names = archive.namelist()
            self.assertTrue(any(name.endswith("/coletando-documentacao-tdn-protheus/SKILL.md") for name in names))
            self.assertFalse(any("tdn_protheus_mcp" in name or "/tdn-cache/" in name or "/saida-local/" in name or name.endswith(".jsonl") or "/.venv/" in name for name in names), names)
